...
from .config import DB_PATH, DEFAULT_DELAY, LXVI_VOTACION_URL_TEMPLATE
...
class SenadoCongresoPipeline:
    ...
    def scrape_range(self, start: int, end: int, limit: Optional[int] = None) -> dict:
...
Ejemplos:
  python -m senado.scraper --range 1 5070
  python -m senado.scraper --range 1 100 --limit 10
  python -m senado.scraper --test-id 1
  python -m senado.scraper --test-id 5065
  python -m senado.scraper --init-schema
  python -m senado.scraper --stats
