...
def determinar_resultado(
    pro_count: int, contra_count: int, requirement: str = "mayoria_simple"
) -> str:
    """Determina el resultado de una votación según el tipo de mayoría.

    - mayoria_simple: aprobada si a_favor > en_contra
    - mayoria_calificada: aprobada si a_favor >= 2/3 de presentes
    ...
    if requirement == "mayoria_calificada":
        # Nota: para mayoria calificada necesitamos conteo de presentes,
        # pero el Senado no siempre lo proporciona por separado.
        # Fallback a mayoria_simple si no hay datos de presentes.
        return (
            "aprobada"
            if pro_count > contra_count
            else ("rechazada" if pro_count < contra_count else "empate")
        )
    else:
        if pro_count > contra_count:
            return "aprobada"
...
def inferir_genero(nombre: str) -> Optional[str]:
