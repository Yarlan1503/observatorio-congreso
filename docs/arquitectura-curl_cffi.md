...
def test_waf_detection_size():
    ...
    assert not client._is_waf_response(small_html, 200)

def test_waf_detection_status():
    """Detecta status codes de bloqueo."""
    client = SenateClientWithLegacyHeaders(use_cache=False)
    
    assert client._is_waf_response("<html></html>", 403)
    assert client._is_waf_response("<html></html>", 429)
    assert not client._is_waf_response("<html></html>", 200)
```
