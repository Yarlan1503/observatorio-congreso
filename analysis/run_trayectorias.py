...
def main():
    parser = argparse.ArgumentParser(
        description="Análisis de Trayectorias Individuales del Congreso",
    )
    parser.add_argument(
        "--camara",
    ...
    if args.camara == "ambas":
        for camara in ["diputados", "senado"]:
            logger.info("\n" + "=" * 60)
            logger.info("=== TRAYECTORIAS — %s ===", camara.upper())
            logger.info("=" * 60)
            run_trayectorias(camara, args.output_dir)
    else:
        logger.info("=" * 60)
        logger.info("=== TRAYECTORIAS — %s ===", args.camara.upper())
        logger.info("=" * 60)
        run_trayectorias(args.camara, args.output_dir)
