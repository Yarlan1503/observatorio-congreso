...
from scraper_congreso.senadores.models import (
    SenCountPorPartido,
    SenVotacionDetail,
    SenVotoNominal,
)
from scraper_congreso.utils.text_utils import MESES_ES
...
def parse_votacion_page(html: str, senado_id: int) -> SenVotacionDetail:
    ...
    fecha_ddmmyyyy = ""
    if fecha_raw:
        match = re.search(r"(\d{1,2})\s+de\s+(\w+)\s+de\s+(\d{4})", fecha_raw, re.IGNORECASE)
        if match:
            dia = match.group(1).zfill(2)
            mes_nombre = match.group(2).lower()
            año = match.group(3)
    ...
    return SenVotacionDetail(
        ...
        periodo=legislature if legislature else periodo,
...
def _voto_to_count_key(voto: str) -> str | None:
